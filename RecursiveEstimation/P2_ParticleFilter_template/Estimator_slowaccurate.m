...
N_particles = 100; % obviously, you will need more particles than 10.
...
% Posterior Update:
zm = intersection1(xm,ym,phim,km,estConst);
beta = zeros(N_particles,N_particles);
for i =1:N_particles
    for j = 1:N_particles
        beta(i,j) = measureNoise(sens-zm(i,j),estConst.epsilon);
    end
end
sum1 = sum(beta,1);
sum2 = sum(beta,2);
...
for i =1:N_particles
    k1 = find(s1>r1(i),1);
    k2 = find(s2>r2(i),1);
    tmp(:,i) = [xm(k2);ym(k2);phim(k2);km(k1)];
end
...
function z = intersection1(xm,ym,phim,km,p)
    xp = p.contour(:,1);
    yp = p.contour(:,2);
    z = Inf(size(km,2),size(xm,2));
    for i=1:size(km,2)
        xp(8:9,1)=km(i);
        r = max(xp)-min(xp)+max(yp)-min(yp);
        poly = polyshape(xp.',yp.');
        for j = 1:size(xm,2)
            lineseg = [xm(j),ym(j);xm(i)+r*cos(phim(j)),ym(i)+r*sin(phim(j))];
            [in,~]=intersect(poly,lineseg);
            z(i,j)= sqrt((in(1,1)-in(2,1))^2+(in(1,2)-in(2,2))^2);
        end
    end
end
...
%                     end
%                 end
%                 if (sqrt((tmp_x-xm(j))^2+(tmp_y-ym(j))^2)<z(m,j)&(xmin(i)<=tmp_x)&(tmp_x<=xmax(i))&(ymax(i)>=tmp_y)&(ymin(i)<=tmp_y))
%                     z(m,j) = sqrt((tmp_x-xm(j))^2+(tmp_y-ym(j))^2);
%                     x(j) = tmp_x;
%                     y(j) = tmp_y;
%                 end
% 
